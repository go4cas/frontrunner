...
**Status:** v2 closed (2026-07-19) · next phase unscheduled
**Version:** 3.16 · **Date:** 2026-07-19 · **Owner:** Cas (`go4cas`)
...
### v2 — self-containment, motion, presets *(CLOSED 2026-07-19 — all 5 items shipped)*
...
- [x] **WebM export** — canvas `captureStream()` + `MediaRecorder`, driven by the real `Playback` engine (forced non-looping) rather than reimplemented timing, so holds/easing/pauses are correct for free. Images are unconditionally embedded before recording (the only working option — see incident 15) and a `<style>` carrying both the collected stylesheet and the theme's live CSS-variable values is injected into each captured frame (see incident 16), or text renders invisible. A **9:16 portrait preset** renders into a separate, genuinely-laid-out-but-off-screen `svg` + fresh `Painter` at true portrait proportions, rather than squeezing the live landscape view.
- [x] **Templates** — bundle Layout + Theme + a curated "look and feel" subset of Settings into one click. Three built-ins (Classic Race, Broadcast Bold, Minimal Print); custom templates save via the same `listCustom`/`saveCustom` mechanism Layout and Theme presets already use.
- [x] **`timeScale: "proportional"`** — animation pacing reflects real elapsed time between periods rather than treating every gap as equal. Isolated entirely to `engine.js`: `Playback`'s clock stays a plain linear 0..(P-1) value (timing/holds/looping all untouched); only the *mapping* from that clock to which two periods to interpolate between changes, via a proportional position scale computed once per dataset in `precompute()`. Falls back silently to equal spacing when periods aren't all real, chronological dates.

*PRD 3.16 note: this is the deferred phase-boundary refresh for v2's close — decisions and incidents logged inline across 3.8–3.15 during the build are now consolidated under the closed roadmap above, rather than left scattered.*
...
- **PRD 3.16 (v2.0.0):** v2 CLOSED — all 5 items shipped (JSON input, raw-data toggle+trim, image embedding, WebM export incl. 9:16 portrait, Templates, proportional time scale); phase-boundary consolidation done; next phase unscheduled.
...
- **Proportional time scale never touches Playback's clock (v2.0.0).** The clock stays a plain linear 0..(P-1) value exactly as before — holds, easing, looping, speed are all untouched. Only the *mapping* from that clock to which two periods to interpolate between changes, computed once per dataset in `precompute()` rather than every frame. This isolated the entire feature to `engine.js` and meant every existing timing test kept passing unchanged, since equal-mode math is now a special case of the same code path (verified: the old `Math.round(tc)` period-label logic and the new `frac < 0.5 ? i : j` logic are provably equivalent in equal mode).
...
17. **Header Layout/Theme dropdowns went stale on any programmatic change (v1.9.7).** `setLayout()`/`setTheme()` update the live painter and stage correctly, but only the header select's OWN change handler ever called `syncSelect()` to update its displayed value — any OTHER caller (Templates' apply, in this case) changed the actual race but left the header dropdown showing the previous choice. Caught by an e2e test asserting the header select's value after applying a template — worth noting explicitly: this was the *same test* that failed a round earlier on a genuine selector collision (its own bug, not the product's), and this time it caught a real one. Fixed by having both setters call `syncSelect()` themselves, so any future caller gets this for free.
